...
Huye – campus.
Business Information Technology Level II.
Academic year: 2024 - 2025.
Name: Bahati Hakizimana.
Reg number: 223010852.
Final project:
...
This hotel room booking system is designed using object-oriented principles, primarily focusing on polymorphism and dynamic memory management. It fulfills the requirements by defining a Date struct, creating an abstract Room class with derived room types, dynamically managing reservations with pointer arithmetic, and handling dynamic arrays of Room pointers.
The system begins by defining a simple struct Date containing integer members for day, month, and year to represent booking dates. Each Room object maintains its own dynamic array of Date objects (Date* bookedDates), which allows the system to store multiple bookings flexibly. The size of this array can change dynamically as reservations are added.
An abstract base class named Room is declared, which includes two pure virtual functions:
bool isAvailable(const Date*) checks if the room is available on a given date.
void book(const Date*) adds a booking for the specified date.
SingleRoom and DoubleRoom classes inherit from Room and provide concrete implementations of isAvailable and book. Each derived class manages its own bookedDates dynamic array. To check availability, pointer arithmetic is used to iterate through the array and detect conflicts by comparing the requested booking date against existing bookings. If no conflicts are found, booking is allowed and the new date is appended using pointer arithmetic manipulation.

The system maintains a dynamic array of pointers to Room objects called rooms (Room** rooms). This array can grow or shrink dynamically as rooms are added or removed. To resize this array, functions addRoom(Room*) and removeRoom(int roomId) are implemented. These functions allocate new arrays with the updated size, copy existing pointers using pointer arithmetic, and manage deletion of the old arrays to avoid memory leaks. This approach provides the flexibility to manage an arbitrary number of rooms during runtime.
The system uses pointer arithmetic extensively on the bookedDates arrays within each Room. This enables efficient traversal of bookings to check for conflicts and to append new bookings dynamically. By incrementing pointers and dereferencing them, the system can access and manipulate bookings as if they are elements of an array, while the actual storage is dynamically allocated.

Below is the full annotated code with comments detailing the purpose of each line.
=================================================================================
